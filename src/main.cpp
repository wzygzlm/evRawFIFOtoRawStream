...
void EVRawFIFOToRawStream(volatile ap_uint<16> eventFIFOIn, ap_uint<1> eventFIFODataValid, status_t *status,
		...
		hls::stream< ap_uint<16> > &rawDataStream)
{
#pragma HLS INTERFACE s_axilite register port=status bundle=config
#pragma HLS INTERFACE axis register both port=rawDataStream
...
void EVRawStreamToXYTSStream(volatile ap_uint<16> eventFIFOIn, ap_uint<1> eventFIFOEmpty, ap_uint<1> *eventFIFORd,
		...
		hls::stream< ap_uint<8> > &rawDataStreamOut)
{
